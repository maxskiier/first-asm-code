...
ZPINIT3:
  	...
  	BEQ ZPINIT4 ; if value loaded is $0, start loading string data
  	STA IRQVALUES,X ; otherwise, store that value
 	INX
  	JMP ZPINIT3 ; and loop
ZPPREP4: ; initlialize charprint loop
  	LDX #$00
ZPINIT4:
	LDA LOADSTRINGDATA,X ; again, load values
	BEQ INITRETURN ; return from subroutine if valued loaded is 0
	STA $5000,X ; if it wasnt 0, store that value at hex 5000 indexed by X
	INX ; increment X
	JMP ZPINIT4 ; loop
INITRETURN: ; simple return function
  	RTS
